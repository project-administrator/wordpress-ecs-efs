...
variable "rds_create" {
  description = "Create the RDS instance with this module? If no, then set the next variable with DB endpoint"
  type        = bool
  default     = true
}

variable "rds_endpoint" {
  description = "Provide RDS endpoint in case rds_create is false"
  type        = string
  default     = "NONE"
}
...
locals {
  ...
  rds_cluster_instance_count     = var.rds_create ? var.rds_cluster_instance_count : 0
