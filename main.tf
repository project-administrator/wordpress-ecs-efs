...
resource "aws_ecs_task_definition" "wordpress" {
  ...
  container_definitions = templatefile(
    "${path.module}/wordpress.tpl",
    {
      ...
      wordpress_db_host          = var.rds_create ? aws_rds_cluster.wordpress[0].endpoint : var.rds_endpoint
...
resource "aws_rds_cluster_instance" "wordpress" {
  ...
  cluster_identifier   = aws_rds_cluster.wordpress[0].id
  engine               = aws_rds_cluster.wordpress[0].engine
  engine_version       = aws_rds_cluster.wordpress[0].engine_version
